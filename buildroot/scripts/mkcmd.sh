...
function set_build_info()
{
	...
	echo "export LICHEE_ARCH=${LICHEE_ARCH}" >> .buildconfig
...
function init_kern_ver()
{
	...
	elif [ "x${LICHEE_CHIP}" = "xsun8iw6p1" -o "x${LICHEE_CHIP}" = "xsun8iw7p1" -o "x${LICHEE_CHIP}" = "xsun8iw8p1" -o "x${LICHEE_CHIP}" = "xsun9iw1p1" ] ; then
...
function select_arch()
{
	...
	if [ "x${CONFIG_ALL}" = "x${FLAGS_TRUE}" ]; then
		...
		printf "select arch by kernel version and chip\n"
		if [ -n "`echo ${LICHEE_CHIP} | grep "sun5[0-9]i"`" ] && \
			[ "x${LICHEE_KERN_VER}" = "xlinux-3.10" ]; then
			export LICHEE_ARCH="arm64"
		else
