...
DEFINE_string 'kernel' 'linux-3.4' 'kernel to build, e.g. linux-3.3' 'k'
...
if ! init_chips ${chip} ; then
    mk_error "invalid chip '${chip}'  need to set chip like -p [chip]_[platform]"
    exit 1
fi

if ! init_platforms ${platform} ; then
	mk_error "invalid platform '${platform}'  need to set platform like -p [chip]_[platform]"
	exit
fi
